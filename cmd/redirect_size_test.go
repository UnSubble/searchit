...
func TestRedirectFinalSizeReporting(t *testing.T) {
	...
	t.Run("Scenario 1 & 2: Single 302 redirect with --follow-redirects reports final 200 and final body size (8432 B)", func(t *testing.T) {
		...
		if !strings.Contains(outStr, "[+] 200 - 8432 B") {
			t.Errorf("expected '[+] 200 - 8432 B' in output file, got: %s", outStr)
		}
	})

	t.Run("Scenario 1 & 2 (Human Readable): Single 302 redirect with --follow-redirects -R reports final 200 and final body size (8.2 KB)", func(t *testing.T) {
		...
		content, _ := os.ReadFile(outFile)
		outStr := string(content)
		if !strings.Contains(outStr, "[+] 200 - 8.2 KB") {
			t.Errorf("expected '[+] 200 - 8.2 KB' in output file with -R, got: %s", outStr)
		}
	})
	...
	t.Run("Scenario 4: Redirect chain (301 -> 302 -> 200) reports final 200 status and final body size (8432 B)", func(t *testing.T) {
		...
		outStr := string(content)
		if !strings.Contains(outStr, "[+] 200 - 8432 B") {
			t.Errorf("expected '[+] 200 - 8432 B' for redirect chain in output file, got: %s", outStr)
		}
	})

	t.Run("Scenario 4 (Human Readable): Redirect chain (301 -> 302 -> 200) with --human-readable reports final 200 (8.2 KB)", func(t *testing.T) {
		...
		outStr := string(content)
		if !strings.Contains(outStr, "[+] 200 - 8.2 KB") {
			t.Errorf("expected '[+] 200 - 8.2 KB' for redirect chain in output file with --human-readable, got: %s", outStr)
		}
	})
}
