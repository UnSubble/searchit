...
func TestChaos_Scans(t *testing.T) {
	...
	t.Run("concurrent cancellation and timeout storm", func(t *testing.T) {
		...
		for i := 0; i < 50; i++ {
			...
			manager.Run(runCtx, runCtx, cfg.URLs, cfg.Threads, func(r engine.Result) {}, nil)
			wg.Wait()
		}
	})
}
	...
	for i := 0; i < runs; i++ {
		...
		func() {
			...
			if recursive {
				...
				manager.Run(ctx, ctx, []string{srv.URL}, workers, func(r engine.Result) {}, nil)
			} else {
				// Fuzzing loop
